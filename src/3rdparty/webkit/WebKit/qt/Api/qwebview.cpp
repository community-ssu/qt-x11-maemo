...
public:
    QWebViewKineticScroller() : QObject(), QAbstractKineticScroller(), m_view(0), m_ignoreEvents(false) {}
...
protected:
    ...
    {
        QMouseEvent cmem(QEvent::MouseMove, QPoint(-INT_MAX, -INT_MAX),
                         Qt::LeftButton, QApplication::mouseButtons() | Qt::LeftButton,
                         QApplication::keyboardModifiers());
        sendEvent(m_view, &cmem);

        QMouseEvent cmer(QEvent::MouseButtonRelease, QPoint(-INT_MAX, -INT_MAX),
                         Qt::LeftButton, QApplication::mouseButtons() & ~Qt::LeftButton,
                         QApplication::keyboardModifiers());
    ...
    QWebFrame* scrollingFrameAt(const QPoint& pos) const
    {
        ...
        if (m_view) {
            QWebFrame* mainFrame = m_view->page()->mainFrame();
            hitFrame = mainFrame->hitTestContent(pos).frame();
