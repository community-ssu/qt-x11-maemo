...
{
    return gtkWidgetMap()->value(path);
}
...
QFont QGtkStylePrivate::getThemeFont(const QString &logicalFontName)
{
    QFont font;
    GtkStyle *style;

    if (!logicalFontName.isEmpty() && gtk_init)
        style = gtk_rc_get_style_by_paths(gtk_settings_get_default(), logicalFontName.toUtf8().constData(), NULL, G_TYPE_NONE);
    else
        style = gtkStyle();

    if (style && (qApp->desktopSettingsAware() || !logicalFontName.isEmpty()))
    {
